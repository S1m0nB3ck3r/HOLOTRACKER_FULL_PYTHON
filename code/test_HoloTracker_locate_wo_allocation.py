...
"""
Filename: test_HoloTracker_locate_wo_allocation.py

Description:
Version optimisée de test_HoloTracker_locate.py utilisant les assignations in-place
pour éviter les réallocations GPU et améliorer les performances.
...
# print("=== VERSION OPTIMISÉE SANS RÉALLOCATIONS ===")
# print("Utilise les assignations in-place array[:] = ... au lieu de array = ...")
# print()
...
for image in os.listdir(path):
    if (image.split('.')[-1].lower() == type_image.lower()):
        ...
        # OPTIMISATION 1: Réutilise h_holo au lieu de créer un nouveau
        h_holo[:] = read_image(os.path.join(path,image), cam_nb_pix_X, cam_nb_pix_Y)
        if display_images:
            display(h_holo)

        #Remove mean image - OPTIMISATION 2: assignation in-place
        h_holo[:] = h_holo / h_mean_holo
        min_val = h_holo.min()
        max_val = h_holo.max()
        img = Image.fromarray((h_holo - min_val) * 255 / (max_val - min_val))
        
        # OPTIMISATION 3: Réutilise d_holo au lieu de créer un nouveau
        ...
        # focus on the volume (INPLACE) - DÉJÀ OPTIMISÉ DANS L'ORIGINAL
